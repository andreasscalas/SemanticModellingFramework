...
namespace ShapeOp {
    ...
    int AndreasSolver::addConstraint(const std::shared_ptr<Constraint>& c) {
        constraints_.push_back(c);
        return static_cast<int>(constraints_.size() - 1);
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline std::shared_ptr<Constraint>& AndreasSolver::getConstraint(int id) {
            return constraints_[id];
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline int AndreasSolver::addForces(const std::shared_ptr<Force>& f) {
            forces_.push_back(f);
            return static_cast<int>(forces_.size() - 1);
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline std::shared_ptr<Force>& AndreasSolver::getForce(int id) {
            return forces_[id];
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline void AndreasSolver::setPoints(const Matrix3X& p) {
            points_ = p;
    }

    ///////////////////////////////////////////////////////////////////////////////
    inline void AndreasSolver::setTimeStep(Scalar timestep) {
            delta_ = timestep;
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline void AndreasSolver::setDamping(Scalar damping) {
            damping_ = damping;
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline const Matrix3X& AndreasSolver::getPoints() {
            return points_;
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline void AndreasSolver::setCagePoints(const Matrix3X& p)
    {
            this->cage_points_ = p;
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline void AndreasSolver::setBCoords(const MatrixXX& b)
    {
            this->Bt_ = b.transpose();
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline const Matrix3X& AndreasSolver::getCagePoints()
    {
            return cage_points_;
    }
    ///////////////////////////////////////////////////////////////////////////////
    inline bool AndreasSolver::initialize(bool dynamic, Scalar masses, Scalar damping, Scalar timestep)
    {
        ...
        MatrixXX B_ = Bt_.transpose();
        N_ = (Bt_ * At_ * A * B_).sparseView(0.0001).eval();
        //N_ = tmp3;//(Bt * At_.toDense() * A.toDense() * B_).sparseView();
                //if (dynamic_) N_ += M_;
    ...
    ///////////////////////////////////////////////////////////////////////////////
    inline bool AndreasSolver::solve(unsigned int iteration)
    {
        ...
        for (unsigned int it = 0; it < iteration; ++it) {
            SHAPEOP_OMP_PARALLEL
            {
                ...
                SHAPEOP_OMP_FOR for (int i = 0; i < 3; ++i)
                    cage_points_.row(i) = solver_->solve(Bt_ * At_ * projections_.row(i).transpose()/* + M_ * momentum_.row(i).transpose()*/,
                                                         cage_points_.row(i).transpose()).transpose(); //TODO: should At_ nd M_ be row major? Temporary variables?
            }
        }
        /*if (dynamic_) {
            SHAPEOP_OMP_PARALLEL
        ...
        this->points_ = this->cage_points_ * Bt_;
        return solver_->info() == Eigen::Success;
    }
}
