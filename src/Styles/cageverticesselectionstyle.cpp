...
void CageVerticesSelectionStyle::OnMiddleButtonDown()
{
    if(Interactor->GetControlKey())
    {
        ...
        if(cellID >= 0 && cellID < this->model->T.numels()){
            ...
            for(unsigned int i = 0; i < model->getDAnnotations().size(); i++)
            {
                SurfaceAnnotation* a = dynamic_cast<SurfaceAnnotation*>(model->getDAnnotations()[i]);
                if(a->isTriangleInAnnotation(model->getTriangle(cellID)))
                    selectedAnnotations.push_back(dynamic_cast<DrawableAnnotation*>(a));
            }
