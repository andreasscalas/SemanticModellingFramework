...
bool isPointInSegment(Point p, Point a, Point b)
{
    double l = (b - a).length();
...
void MeasureStyle::manageCaliberMovement()
{
    ...
    while(boundaryPolydata->GetLines()->GetNextCell(idList))
        for(vtkIdType i = 0; i < idList->GetNumberOfIds(); i++)
        {
            ...
            if(isPointInSegment(projected, projectedExtreme1, projectedExtreme2))
            {
    ...
    measurePath.push_back(h->getNearestNeighbors(extremePoint1, 1));
    measurePath.push_back(h->getNearestNeighbors(extremePoint2, 1));
...
void MeasureStyle::manageBoundingMovement()
{
    ...
    for(unsigned int i = 0; i < mesh->getAnnotations().size(); i++)
        ...
        {
            selected = true;
            std::vector<IMATI_STL::Vertex*> involved = mesh->getDAnnotations()[i]->getInvolvedVertices();
...
//    Point defaultDirection(1,0,0);
//    double cos_angle = (*boundingDirection) * defaultDirection;
//    double angle = acos(cos_angle);
//    if(cos_angle < 0)
//        angle += M_PI;
//    Point axis = ((*boundingDirection) & defaultDirection);
//    axis.normalize();

//    transform->RotateWXYZ(angle, axis.x, axis.y, axis.z);
//    Point scalingVector = *boundingDirection * ((*boundingEnd) - (*boundingBegin)).length();
//    transform->Scale(scalingVector.x, scalingVector.y, scalingVector.z);
//    transform->Translate(boundingBegin->x, boundingBegin->y, boundingBegin->z);
//    actor->SetUserTransform(transform);
//    measureAssembly->AddPart(actor);
...
void MeasureStyle::reset()
{
    ...
    measureAssembly = vtkSmartPointer<vtkPropAssembly>::NewInstance(measureAssembly);
    cellPicker = vtkSmartPointer<vtkCellPicker>::NewInstance(cellPicker);
    cellPicker->SetPickFromList(1);
    cellPicker->AddPickList(mesh->getMeshSurfaceActor());
    updateView();
...
void MeasureStyle::updateView()
{
    ...
    for(unsigned int i = 0; i < mesh->getDAnnotations().size(); i++)
        ...
        {
            dynamic_cast<DrawableAttribute*>(mesh->getAnnotations()[i]->getAttributes()[j])->setRenderer(meshRenderer);
            dynamic_cast<DrawableAttribute*>(mesh->getAnnotations()[i]->getAttributes()[j])->draw(measureAssembly);
        }
...
void MeasureStyle::OnLeftButtonDown()
{
    ...
    if(this->Interactor->GetControlKey()){
        ...
        if(measureType == MeasureType::BOUNDING || measureType == MeasureType::CALIBER)
        {
            ...
            for(unsigned int i = 0; i < mesh->getAnnotations().size(); i++){
                if(mesh->getAnnotations()[i]->isPointInAnnotation(v) && mesh->getDAnnotations()[i]->getSelected()){
                    if(!measureStarted)
                    {
                        measurePath.clear();
                        measure = 0.0;
