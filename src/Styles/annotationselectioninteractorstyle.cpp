...
void AnnotationSelectionInteractorStyle::OnLeftButtonDown()
{
    if(this->Interactor->GetControlKey()){
        ...
        if(cellID > 0 && cellID < this->mesh->T.numels()){
            ...
            Vertex* v_ = t->v1();
            Vertex* v;
            vector<DrawableAnnotation*> annotations = mesh->getDAnnotations();
            ...
            for(unsigned int i = 0; i < annotations.size(); i++)
                if(annotations[i]->isPointInAnnotation(v))
                    selected.push_back(annotations[i]);
            ...
            if(selected.size() != 0){
                ...
                vector<DrawableAnnotation*>::iterator ait = std::find(annotations.begin(), annotations.end(), selectedAnnotation);

                if(ait != annotations.end()){
                    DrawableAnnotation* annotation = static_cast<DrawableAnnotation*>(*ait);
...
void AnnotationSelectionInteractorStyle::resetSelection()
{
    for(unsigned int i = 0; i < mesh->getAnnotations().size(); i++)
        mesh->getDAnnotations()[i]->setSelected(false);
}
