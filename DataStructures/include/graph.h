...
public:
    ...
    std::vector<Arc<T>*> getArcsFromFletching(Node<T>* n);
    std::vector<Arc<T>*> getArcsFromTip(Node<T>* n);
    Arc<T>* getArcFromEndpoints(Node<T>* n1, Node<T>* n2);
...
bool Graph<T>::removeArc(Arc<T> *a)
{
    ...
    for(it = this->arcs.begin(); it != this->arcs.end(); it++)
        if(*it == a)
            break;
    //it = std::find(this->arcs.begin(), this->arcs.end(), a);
    if(it != this->arcs.end()){
...
template<class T>
std::vector<Arc<T> *> Graph<T>::getArcsFromFletching(Node<T> *n)
{
    std::vector<Arc<T> *> arcs;
...
template<class T>
std::vector<Arc<T> *> Graph<T>::getArcsFromTip(Node<T> *n)
{
    std::vector<Arc<T> *> arcs;
    for(unsigned int i = 0; i < this->arcs.size(); i++)
...
template<class T>
Arc<T> *Graph<T>::getArcFromEndpoints(Node<T> *n1, Node<T> *n2)
{
