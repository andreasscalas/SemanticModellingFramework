...
bool Pipeline::head_cpc_fitting()
{
    ...
    std::string fn_vw_face_and_ears_fitting = cfg->tmpl().fn_vw_face_and_ears_fitting;
    std::string fn_mapping_body2head = cfg->tmpl().fn_mapping_body2head;
    if (! fn_vw_face_and_ears_fitting.empty())
    {
        if (fn_mapping_body2head.empty())
        {
            template_fit.load_vertexweighting(fn_vw_face_and_ears_fitting, "v:fitting_weight", false);
        }
        else
        {
            template_fit.load_vertexweighting(fn_vw_face_and_ears_fitting, fn_mapping_body2head, "v:fitting_weight", false);
        }
    }
    ...
    if (cfg->head().dbl_mouth_shut_weight > 0.0)
    {
        std::string fn_sel_mouth_shut = cfg->tmpl().fn_sel_mouth_shut;

        Energy_term* E_mouth_shut = E.energy_term_list().get_term(surface_mesh::ET_MOUTH_SHUT);
        if (fn_mapping_body2head.empty())
        {
            template_fit.load_mouth_shut_selection(fn_sel_mouth_shut);
        }
        else
        {
            template_fit.load_mouth_shut_selection(fn_sel_mouth_shut, fn_mapping_body2head);
        }

        E_mouth_shut->set_weight(cfg->head().dbl_mouth_shut_weight);
    }

    if (cfg->head().dbl_ears_lm_weight > 0.0)
    {
        //activate energy
        Energy_term* E_ears = E.energy_term_list().get_term(surface_mesh::ET_EARS_LM);
        E_ears->set_weight(cfg->head().dbl_ears_lm_weight);
    }

    if (cfg->head().dbl_keep_vertices_weight > 0.0)
    {
        std::string fn_sel_keep_vertices = cfg->tmpl().fn_sel_keep_vertices;
        if (fn_mapping_body2head.empty())
        {
            if (!template_fit.load_selection_to_keep(fn_sel_keep_vertices))
            {
                std::cerr << "Pipeline: Unable to load vertices to keep. Filename: \"" << fn_sel_keep_vertices << "\"" << std::endl;
                return false;
            }
        }
        else
        {
            if (!template_fit.load_selection_to_keep(fn_sel_keep_vertices, fn_mapping_body2head))
            {
                std::cerr << "Pipeline: Unable to load vertices to keep. Filename: \"" << fn_sel_keep_vertices << "\"" << std::endl;
                return false;
            }
        ...
        Energy_term* E_keep = E.energy_term_list().get_term(surface_mesh::ET_KEEP_VERTICES);
        E_keep->set_weight(cfg->head().dbl_keep_vertices_weight);
    }

    // load vertices to omit during fitting
    std::string fn_sel_vertices_to_omit = cfg->tmpl().fn_sel_vertices_to_omit;
    if (fn_mapping_body2head.empty())
    {
        if (!template_fit.load_selection_to_omit(fn_sel_vertices_to_omit))
        {
            std::cerr << "Pipeline: Unable to load vertices to omit. Filename: \"" << fn_sel_vertices_to_omit << "\"" << std::endl;
            return false;
        }
    }
    else
    {
        if (!template_fit.load_selection_to_omit(fn_sel_vertices_to_omit, fn_mapping_body2head))
        {
            std::cerr << "Pipeline: Unable to load vertices to omit. Filename: \"" << fn_sel_vertices_to_omit << "\"" << std::endl;
            return false;
        }
