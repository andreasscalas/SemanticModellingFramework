...
//twaltema: helper function to read a specific material from a mtllib file
bool read_material(Surface_mesh& mesh, const std::string& filename, const std::string& mtllib_fname, const std::string& mtl_name)
{
    ...
    std::string full_filename = filepath + mtllib_fname;

    std::stringstream ss;
    std::string line,str;

    std::ifstream ifs(full_filename.c_str());

    if (!ifs)
        return false;

    //just find the right material entry
    while (ifs.good())
    {
        std::getline(ifs, line);
        if (line.find("newmtl") == 0)
            if (line.find(mtl_name.c_str()) != std::string::npos)
                break;
    }

    //next lines contain values for desired material
    while (ifs.good())
    {
        std::getline(ifs,line);

        // only read selected material; break at next material
        ...
        {
            ss >> str;
            Surface_mesh::Mesh_property<std::string> tex_name = mesh.mesh_property<std::string>("m:texturename");
            tex_name[0] = filepath + str;
        }
        else if (str == "map_Ks")
        {
            ss >> str;
            Surface_mesh::Mesh_property<std::string> tex_name = mesh.mesh_property<std::string>("m:texturename_spec");
            tex_name[0] = filepath + str;
        }
        else if (str == "map_bump" || str == "bump")
        {
            ss >> str;
            Surface_mesh::Mesh_property<std::string> tex_name = mesh.mesh_property<std::string>("m:texturename_nm");
            tex_name[0] = filepath + str;
        }
    }
...
bool read_obj(Surface_mesh& mesh, const std::string& filename)
{
    ...
    std::string mtllib_name; // name of the mtllib
    ...
    while(in && !feof(in) && fgets(s, 200, in))
    {
        ...
        else if (strncmp(s, "mtllib", 6) == 0)
        {
            if (sscanf(s,"mtllib %s", tmp))
            {
                //twaltema: save mtllib filename for later use
                mtllib_name = tmp;
            }
        }

        else if (strncmp(s, "usemtl", 6) == 0)
        {
            if (sscanf(s, "usemtl %s", tmp))
            {
                //twaltema: read material with name tmp from mtllib file
                read_material(mesh,filename, mtllib_name, tmp);
            }
        }
        ...
        else if (strncmp(s, "f ", 2) == 0)
        {
          ...
          if(with_tex_coord)
          {
              if (!tex_coords)
              {
                  tex_coords = mesh.halfedge_property<Texture_coordinate>("h:texcoord", Texture_coordinate(0.0f));
              }
