#ifndef LEFT_HAND_CONFIG_H
#define LEFT_HAND_CONFIG_H
...
struct Left_hand_config : public Base_config
{
    ...
    virtual void init(const std::vector<std::string>& argv, std::map<std::string, std::string>& data)
    {
        ...
        {
            if (argv[i] == "-l" && argv.size() > i+1)
            {
                dir_project = argv[i+1];
                if (dir_project.back() != '/')
                    dir_project.push_back('/');

                b_has_left_hand_scan = true;
            }
        }

        if (!b_has_left_hand_scan) return;
